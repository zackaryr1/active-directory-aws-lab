...
AD DS role promotion and forest creation, DNS server and forward-lookup zone management, OU hierarchy design, global security group creation and membership, AGDLP nesting for RDP access delegation, domain-level account lockout policy, password reset and account unlock, GPO creation and linking, Block Inheritance troubleshooting, `gpresult /r`, `gpresult /h` HTML reports, `gpupdate /force`, Windows Firewall profile awareness, Windows Server 2022 administration, ADUC, GPMC, PowerShell AD module cmdlets (`New-ADUser`, `Set-ADUser`, `Get-ADUser`, `Set-ADAccountPassword`, `Unlock-ADAccount`, `Move-ADObject`, `Add-LocalGroupMember`), CSV-based bulk user provisioning with `Import-Csv` and `ForEach-Object`, domain join workflow, RDP and NLA interaction with the forced-password-change flag, AWS EC2 instance provisioning, AWS VPC and subnet CIDR planning, AWS security group self-referencing rules for AD traffic, Tier 1 help-desk ticket documentation.
